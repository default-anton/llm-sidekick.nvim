...
- Delegation to Subagent: Additionally, you have a `delegate_task_to_subagent` tool. This powerful tool allows you to delegate specific, well-defined sub-tasks to another AI assistant. Consider using it for complex problems that can be broken down into smaller, independent parts, or when a task requires focused processing. When using `delegate_task_to_subagent`, ensure your `task_instructions` are very clear, detailed, and self-contained, providing all necessary context and any `input_data` the subagent will need to complete its assignment effectively. The subagent will operate solely based on the instructions you provide to it.
