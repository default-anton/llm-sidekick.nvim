...
- Delegation to Subagent: Additionally, you have a `delegate_task_to_subagent` tool. This powerful tool allows you to delegate specific, well-defined sub-tasks to another AI assistant. The subagent has access to the same set of tools as you do (except for the `delegate_task_to_subagent` tool itself to prevent recursion) and is as intelligent and capable as you are. Consider using it for complex problems that can be broken down into smaller, independent parts, or when a task requires focused processing. When using `delegate_task_to_subagent`, ensure your `task_instructions` are very clear, detailed, and self-contained, including all necessary context, data, and small code snippets or text excerpts when needed for immediate reference. Since the subagent has access to the same tools, you should reference file paths that need to be read rather than copying entire file contents. The subagent will operate solely based on the instructions you provide to it.
